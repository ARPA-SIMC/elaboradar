...
//fattore calibraz radar per eventuale rigenerazione dati
...
float resol[2]; 
int x_size,z_size; 
long int ncv=0,ncs=0,np=0; 
float *convective_radius; 
float htbb=-9999., hbbb=-9999.; 
FILE *log_class;
...
{  
...
#ifdef CLASS
  for (k=0; k<NUM_AZ_X_PPI*MAX_BIN;k++ ){
    lista_conv[k][0]=-999;
    lista_conv[k][1]=-999;
    lista_bckg[k][0]=-999;
    lista_bckg[k][1]=-999;
  ...
  if(ier == OK  && ier_test) 
	...
	{ 
	  ...
	  ier_comb=combina_profili(argv[3]); 
...
/*===============================================*/ 
time_t 	NormalizzoData  (time_t time)    /* time è in secondi, itime è un intero che rappresenta il numero intero di intervalli da 5 minuti*/ 
/*===============================================*/ 
