        ...
        const Volume<double>& SD_Z6)
{
    ...
    for(unsigned i=0; i<NUM_AZ_X_PPI; i++) 
    {
        ...
        for(unsigned k=0; k<volume[0].beam_size; k++)
        {
            ...
            if(bin_high == fondo_scala && SD_Z6[el_inf].get(i,k)<= conf_texture_threshold && SD_Z6[el_inf].get(i,k) > 0.01)                     //-----------ANNULLO EFFETTO TEST ANAP
            {
		do_test_AP=false;
                    ...
                    (
                      ...
                      (
                          ...
                          bin_low > MIN_VALUE + 5 
                                  ...
                                  || 
                    (
                      SD_Z6[el_inf].get(i,k) > conf_texture_threshold
//                                  &&  
//                    (
...
//                    )
                   ...
                   )
               {
                   ...
                   if(k < volume[el_up].beam_size && SD_Z6[el_up].get(i,k) <= conf_texture_threshold  && SD_Z6[el_up].get(i,k) >= 0.01){
              ...
              //-----non c'è propagazione anomala:ricopio su tutte e elevazioni il valore di el_inf e correggo il beam blocking,  incremento la statistica beam_blocking, assegno matrice anaprop a 0 nel punto , assegno a 0 indicatore anap nel raggio, assegno elevazione finale e incremento statisica cambio elevazione se el_inf > first_level_static(i, k)-----------
               else
               {
                   ...
                   for (unsigned ii=0; ii<7; ii++){
                       ...
                       if( SD_Z6[el_inf].get(iaz_prox,k) < conf_texture_threshold && SD_Z6[el_inf].get(iaz_prox,k) > 0.01) count_low++;
                       if( k < SD_Z6[el_up].beam_size && SD_Z6[el_up].get(iaz_prox,k) < 1.7*conf_texture_threshold&& SD_Z6[el_up].get(iaz_prox,k) > 0.01) count_high++;
                   }
                   if ( !(SD_Z6[el_inf].get(i,k) < 1.3*conf_texture_threshold && SD_Z6[el_inf].get(i,k) > 0.01 && count_low >=5)) {
                       if ( k >= SD_Z6[el_up].beam_size || !(SD_Z6[el_up].get(i,k) < 1.7* conf_texture_threshold && SD_Z6[el_up].get(i,k) > 0.01 && count_high >=3)){
                           bin_low = fondo_scala;
...
//  LOG_WARN("b@(%3d,%3d) - el_inf %2d  - el_up %2d -low %6.2f - up %6.2f fin %6.2f- cont %3d %1d %1d %6.2f %6.2f %6.2f %6.2f  --  %6.2f %6.2f %6.2f NO_TA-low <fondo",i,k,el_inf,el_up,bin_low,bin_high, volume[0].get(i,k),cont_anap,test_an, flag_anap, MAX_DIF, MIN_VALUE, MAX_DIF_NEXT, MIN_VALUE_NEXT, SD_Z6[el_inf].get(i,k),SD_Z6[el_inf].get((i+1)%NUM_AZ_X_PPI,k) ,SD_Z6[el_inf].get((i-1+NUM_AZ_X_PPI)%NUM_AZ_X_PPI,k));
...

            {
                ...
                for (unsigned ii=0; ii<7; ii++){
                    ...
                    if( SD_Z6[el_inf].get(iaz,k) < conf_texture_threshold && SD_Z6[el_inf].get(iaz,k) > 0.01) count++;
                }
                if ( !(SD_Z6[el_inf].get(i,k) < conf_texture_threshold && SD_Z6[el_inf].get(i,k) >0.01 && count >=5 )) 
                    bin_low = fondo_scala;
