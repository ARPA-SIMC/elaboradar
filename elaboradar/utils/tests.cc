...
using namespace std;
using namespace elaboradar::utils;

const elaboradar::utils::tests::LocationInfo elaboradar_utils_test_location_info;
...
void test_assert_file_exists(WIBBLE_TEST_LOCPRM, const std::string& fname)
{
    if (not sys::fs::exists(fname))
    {
        ...
        elaboradar::utils_test_location.fail_test(ss.str());
    }
}
...
struct TestFileExists
{
    ...
    void check(ELABORADAR_UTILS_TEST_LOCPRM) const;
};
#endif
