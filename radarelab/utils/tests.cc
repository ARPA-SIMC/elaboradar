...
const radarelab::utils::tests::LocationInfo radarelab_utils_test_location_info;
...
void test_assert_file_exists(WIBBLE_TEST_LOCPRM, const std::string& fname)
{
    if (not sys::fs::exists(fname))
    {
        ...
        radarelab::utils_test_location.fail_test(ss.str());
    }
}
...
struct TestFileExists
{
    ...
    void check(RADARELAB_UTILS_TEST_LOCPRM) const;
};
#endif
