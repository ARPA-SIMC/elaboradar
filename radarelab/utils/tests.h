#ifndef RADARELAB_UTILS_TESTS_H
#define RADARELAB_UTILS_TESTS_H
 ...
 */
extern const radarelab::utils::tests::LocationInfo radarelab_utils_test_location_info;
 ...
 */
#define RADARELAB_UTILS_TEST_INFO(name) \
    radarelab::utils::tests::LocationInfo radarelab_utils_test_location_info; \
    radarelab::utils::tests::LocationInfo& name = radarelab_utils_test_location_info
...
#define wassert(...) \
    do { try { \
        __VA_ARGS__ ; \
    } catch (radarelab::utils::tests::TestFailed& e) { \
        e.add_stack_info(__FILE__, __LINE__, #__VA_ARGS__, radarelab_utils_test_location_info); \
        throw; \
    } catch (std::exception& e) { \
        throw radarelab::utils::tests::TestFailed(e, __FILE__, __LINE__, #__VA_ARGS__, radarelab_utils_test_location_info); \
    } } while(0)
...
#define wcallchecked(func) \
    [&]() { try { \
        ...
        e.add_stack_info(__FILE__, __LINE__, #func, radarelab_utils_test_location_info); \
        throw; \
    } catch (std::exception& e) { \
        throw radarelab::utils::tests::TestFailed(e, __FILE__, __LINE__, #func, radarelab_utils_test_location_info); \
    } }()

/**
 * Fail a test with an error message
 */
#define wfail_test(msg) wassert(throw radarelab::utils::tests::TestFailed((msg)))
